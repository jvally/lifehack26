...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    application: {
      extractProduct: vi.fn(async () => ({ passport: { productId: product.id } })),
      evaluateProduct: vi.fn(async () => ({ evaluation: { scoringVersion: "1.0.0" } })),
      startInterview: vi.fn(async () => ({ session: { id: "session-1" } })),
      answerInterview: vi.fn(async () => ({ nextGap: null })),
      simulateProduct: vi.fn(async () => ({ before: {}, after: {} })),
      importMarketSignals: vi.fn(async (signals) => ({ importedCount: signals.length })),
    },
  };
});
...
describe("application service routes", () => {
  it("delegates extraction and evaluation", async () => {
    ...
    expect(mocks.application.extractProduct).toHaveBeenCalledWith(productId);
    expect(mocks.application.evaluateProduct).toHaveBeenCalledWith(productId);
  });
  ...
  it("delegates interview start and validated answers", async () => {
    ...
    expect(mocks.application.startInterview).toHaveBeenCalledWith(productId);
    expect(mocks.application.answerInterview).toHaveBeenCalledWith(
      sessionId,
  ...
  it("delegates simulation and market signal import", async () => {
    ...
    expect(mocks.application.simulateProduct).toHaveBeenCalledWith(
      productId,
      "lightweight road shoes",
    );
    expect(mocks.application.importMarketSignals).toHaveBeenCalledWith([signal]);
  });
});
